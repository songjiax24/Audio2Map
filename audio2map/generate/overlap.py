"""Overlap-window inference: slide fixed-size audio windows and stitch chart notes."""

from __future__ import annotations

import logging
from collections.abc import Callable
from dataclasses import asdict, dataclass, field
from pathlib import Path
from typing import Literal, NamedTuple

import numpy as np
import torch
import torch.nn.functional as F

from audio2map.features.audio.grid import (
    compute_audio_grid,
    load_audio_grid,
    resolve_grid_stem,
    save_audio_grid,
    slice_audio_window,
)
from audio2map.grid import (
    CanonicalTiming,
    chart_event_bar_range,
    ms_to_tick,
    split_tick,
)
from audio2map.tokens import (
    TOKEN_BAR,
    TOKEN_EOS,
    TOKEN_POS_PREFIX,
    TOKEN_ROW_PREFIX,
    RowState,
    build_vocab,
    initial_row_from_active_hold,
    invert_vocab,
    row_state_to_token,
    tokens_to_notes,
)
from audio2map.osu.schema import ManiaNote
from audio2map.generate.decode import ChartDecodeState, bos_initial_prefix
from audio2map.model.config import MAX_DECODER_LEN, WINDOW_BARS
from audio2map.model.model import AudioChartModel
from audio2map.utils.paths import audio_grid_dir

logger = logging.getLogger(__name__)


@dataclass(frozen=True, slots=True)
class OverlapConfig:
    """``context + keep + future = window_bars`` (default 8+4+4=16)."""

    window_bars: int = WINDOW_BARS
    context_bars: int = WINDOW_BARS // 2
    keep_bars: int = WINDOW_BARS // 4
    future_bars: int = WINDOW_BARS // 4
    max_seq_len: int = MAX_DECODER_LEN

    def __post_init__(self) -> None:
        if self.context_bars < 1 or self.keep_bars < 1:
            raise ValueError("context_bars and keep_bars must be >= 1")
        if self.context_bars + self.keep_bars + self.future_bars != self.window_bars:
            raise ValueError(
                f"context+keep+future must equal window_bars "
                f"({self.context_bars}+{self.keep_bars}+{self.future_bars}!={self.window_bars})"
            )


@dataclass(frozen=True, slots=True)
class DecodeConfig:
    temperature: float = 0.8
    top_p: float = 0.95
    top_k: int = 50


@dataclass(frozen=True, slots=True)
class GenerationRangeConfig:
    """``audio_full`` = whole audio; ``reference_chart`` = clip to chart event bars."""

    mode: Literal["audio_full", "reference_chart"] = "audio_full"
    pre_margin_bars: int = 0
    post_margin_bars: int = 4


class InferenceWindow(NamedTuple):
    win_start: int
    win_end: int
    keep_start: int
    keep_end: int
    context_bars: int


@dataclass(slots=True)
class WindowGenerationLog:
    win_start: int
    win_end: int
    keep_start: int
    keep_end: int
    context_bars: int
    is_first_window: bool
    bars_kept: int


@dataclass(slots=True)
class GenerationReport:
    audio_start_bar: int
    audio_end_bar: int
    gen_start_bar: int
    gen_end_bar: int
    chart_start_bar: int | None
    chart_end_bar: int | None
    range_mode: str
    first_note_bar: int | None = None
    last_note_bar: int | None = None
    tail_note_count: int = 0
    audio_ticks_per_window: int = 0
    decode_issues: list[dict[str, str | int]] = field(default_factory=list)
    windows: list[WindowGenerationLog] = field(default_factory=list)

    def to_dict(self) -> dict:
        return asdict(self)


def resolve_generation_bar_range(
    *,
    audio_start_bar: int,
    audio_end_bar: int,
    chart_start_bar: int | None,
    chart_end_bar: int | None,
    cfg: GenerationRangeConfig,
) -> tuple[int, int]:
    """Resolve ``[gen_start_bar, gen_end_bar)`` from audio/chart bounds and ``mode``."""
    if cfg.mode == "reference_chart" and chart_start_bar is not None and chart_end_bar is not None:
        gen_start = max(audio_start_bar, chart_start_bar - cfg.pre_margin_bars)
        gen_end = min(audio_end_bar, chart_end_bar + cfg.post_margin_bars)
        return gen_start, max(gen_start, gen_end)
    return audio_start_bar, audio_end_bar


def overlap_inference_bar_windows(
    gen_start_bar: int,
    gen_end_bar: int,
    cfg: OverlapConfig,
) -> list[InferenceWindow]:
    """Return inference windows covering ``[gen_start_bar, gen_end_bar)``."""
    if gen_start_bar >= gen_end_bar:
        return []

    if gen_end_bar - gen_start_bar <= cfg.window_bars:
        s = gen_start_bar
        return [InferenceWindow(s, s + cfg.window_bars, s, gen_end_bar, 0)]

    windows: list[InferenceWindow] = []
    commit = gen_start_bar
    first = True

    while commit < gen_end_bar:
        if first:
            keep_start = commit
            keep_end = commit + cfg.context_bars
            win_start = gen_start_bar
            win_end = win_start + cfg.window_bars
            context_bars = 0
            first = False
        else:
            keep_start = commit
            win_start = keep_start - cfg.context_bars
            win_end = win_start + cfg.window_bars
            if win_end > gen_end_bar:
                win_end = gen_end_bar
                win_start = gen_end_bar - cfg.window_bars
                keep_end = gen_end_bar
            else:
                keep_end = keep_start + cfg.keep_bars
            context_bars = keep_start - win_start

        if not win_start <= keep_start < keep_end <= win_end:
            raise RuntimeError(
                f"invalid keep/window win=[{win_start},{win_end}) keep=[{keep_start},{keep_end})"
            )
        windows.append(InferenceWindow(win_start, win_end, keep_start, keep_end, context_bars))
        commit = keep_end

    return windows


def sample_next_token_id(
    logits: torch.Tensor,
    allowed: set[int],
    decode: DecodeConfig,
) -> int:
    mask = torch.full_like(logits, float("-inf"))
    for tid in allowed:
        mask[tid] = 0.0
    logits = logits + mask
    if decode.temperature <= 0:
        return int(logits.argmax().item())

    probs = F.softmax(logits / decode.temperature, dim=-1)
    allowed_list = sorted(allowed)
    sub = probs[allowed_list]
    if decode.top_k > 0 and sub.numel() > decode.top_k:
        keep = torch.topk(sub, decode.top_k).indices
        sub = sub[keep]
        allowed_list = [allowed_list[i] for i in keep.tolist()]
    if 0 < decode.top_p < 1.0:
        sorted_probs, order = torch.sort(sub, descending=True)
        cum = torch.cumsum(sorted_probs, dim=0)
        cut = int((cum <= decode.top_p).sum().item())
        cut = max(cut, 0)
        sub = sorted_probs[: cut + 1]
        allowed_list = [allowed_list[i] for i in order[: cut + 1].tolist()]
    sub = sub / sub.sum().clamp(min=1e-12)
    idx = int(torch.multinomial(sub, 1).item())
    return allowed_list[idx]


def initial_row_from_committed_bars(
    committed_bars: dict[int, list[int]],
    *,
    gen_start: int,
    win_start: int,
    vocab: dict[str, int],
) -> RowState:
    """Replay committed chart tokens before ``win_start`` to get hold state at window open."""
    empty = initial_row_from_active_hold([False, False, False, False])
    if win_start <= gen_start:
        return empty

    decode = ChartDecodeState.from_initial_row(
        empty,
        window_bars=max(1, win_start - gen_start),
        vocab=vocab,
    )
    for bar in range(gen_start, win_start):
        for tid in committed_bars.get(bar, []):
            decode.observe_and_advance_bar_if_needed(tid)
    return initial_row_from_active_hold(decode.active_hold)


def _complete_bar_token_ids(ids: list[int], id_to_token: dict[int, str]) -> list[int]:
    """Keep ``<BAR>`` plus complete ``POS ROW`` pairs; drop a trailing POS."""
    if not ids:
        return []
    toks = [id_to_token[i] for i in ids]
    keep = 1
    j = 1
    while j + 1 < len(toks):
        if not toks[j].startswith(TOKEN_POS_PREFIX) or not toks[j + 1].startswith(TOKEN_ROW_PREFIX):
            break
        keep += 2
        j += 2
    return ids[:keep]


def bar_token_ids_by_bar(
    token_ids: list[int],
    id_to_token: dict[int, str],
    *,
    win_start_bar: int,
) -> dict[int, list[int]]:
    """Map absolute bar → ``<BAR> …`` token ids from one window sequence."""
    tokens = [id_to_token[i] for i in token_ids]
    out: dict[int, list[int]] = {}
    current_bar = win_start_bar - 1
    i = 0
    n = len(tokens)
    while i < n and tokens[i] not in (TOKEN_BAR, TOKEN_EOS):
        i += 1
    while i < n and tokens[i] != TOKEN_EOS:
        if tokens[i] != TOKEN_BAR:
            i += 1
            continue
        current_bar += 1
        start = i
        i += 1
        while i < n and tokens[i] not in (TOKEN_BAR, TOKEN_EOS):
            i += 1
        out[current_bar] = _complete_bar_token_ids(token_ids[start:i], id_to_token)
    return out


def extract_bar_token_ids(
    token_ids: list[int],
    id_to_token: dict[int, str],
    *,
    win_start_bar: int,
    bar: int,
) -> list[int]:
    """Token ids for a single absolute bar from one window generation."""
    return bar_token_ids_by_bar(token_ids, id_to_token, win_start_bar=win_start_bar).get(bar, [])


def build_context_prompt_token_ids(
    *,
    committed_bars: dict[int, list[int]],
    win_start: int,
    keep_start: int,
    initial_row: RowState,
    vocab: dict[str, int],
) -> list[int] | None:
    """``<BOS> + <ROW_initial> + committed keep tokens for [win_start, keep_start)``."""
    if keep_start <= win_start:
        return None

    body: list[int] = []
    for bar in range(win_start, keep_start):
        if bar in committed_bars:
            body.extend(committed_bars[bar])
        else:
            logger.warning(
                "missing committed tokens for context bar %d (win_start=%d keep_start=%d)",
                bar,
                win_start,
                keep_start,
            )
            body.append(vocab[TOKEN_BAR])

    prefix = bos_initial_prefix(vocab[row_state_to_token(initial_row)], vocab=vocab)
    return prefix + body


def commit_keep_bar_tokens(
    committed_bars: dict[int, list[int]],
    *,
    token_ids: list[int],
    id_to_token: dict[int, str],
    win_start: int,
    keep_start: int,
    keep_end: int,
) -> list[int]:
    """Commit ``[keep_start, keep_end)``; return keep bars the window did not emit."""
    chunks = bar_token_ids_by_bar(token_ids, id_to_token, win_start_bar=win_start)
    missing: list[int] = []
    for bar in range(keep_start, keep_end):
        bar_ids = chunks.get(bar)
        if bar_ids:
            committed_bars[bar] = bar_ids
        else:
            missing.append(bar)
    return missing


def assemble_chart_token_ids(
    committed_bars: dict[int, list[int]],
    *,
    gen_start: int,
    gen_end: int,
    vocab: dict[str, int],
) -> list[int]:
    """Merge per-bar keep tokens into one chart sequence (no BOS/initial/EOS)."""
    out: list[int] = []
    for bar in range(gen_start, gen_end):
        out.extend(committed_bars.get(bar, [vocab[TOKEN_BAR]]))
    return out


@torch.no_grad()
def generate_window_tokens(
    model: AudioChartModel,
    *,
    audio: torch.Tensor,
    cond_vec: torch.Tensor,
    initial_row: RowState,
    window_bars: int,
    device: torch.device,
    decode: DecodeConfig | None = None,
    max_seq_len: int | None = None,
    prompt_token_ids: list[int] | None = None,
    vocab: dict[str, int] | None = None,
    id_to_token: dict[int, str] | None = None,
) -> list[int]:
    """Generate one window token sequence (including BOS/EOS).

    When ``prompt_token_ids`` is set, it must be
    ``<BOS> + <ROW_initial> + context chart tokens`` for overlap windows.
    """
    decode = decode or DecodeConfig()
    cap = model.max_decoder_len if max_seq_len is None else min(model.max_decoder_len, max_seq_len)
    vocab = vocab or build_vocab()
    id_to_token = id_to_token or invert_vocab(vocab)
    initial_id = vocab[row_state_to_token(initial_row)]
    model.eval()

    if prompt_token_ids:
        token_ids = list(prompt_token_ids)
    else:
        token_ids = bos_initial_prefix(initial_id, vocab=vocab)

    state = ChartDecodeState.from_initial_row(initial_row, window_bars=window_bars, vocab=vocab)
    for tid in token_ids[2:]:
        state.observe(tid)
    if prompt_token_ids is not None and len(token_ids) > 2:
        state.require_bar_after_prompt()

    audio_b = audio.unsqueeze(0).to(device)
    cond_b = cond_vec.unsqueeze(0).to(device)

    while not state.finished and len(token_ids) < cap:
        ids = torch.tensor([token_ids], dtype=torch.long, device=device)
        logits = model.next_token_logits(audio_b, cond_b, ids)[0]
        allowed = state.allowed_token_ids()
        next_id = sample_next_token_id(logits, allowed, decode)
        token_ids.append(next_id)
        state.observe_and_advance_bar_if_needed(next_id)
        if id_to_token[next_id] == TOKEN_EOS:
            break

    if not state.finished:
        raise RuntimeError(
            f"window decode truncated at max_seq_len={cap} "
            f"({state.bars_done}/{window_bars} bars, {len(token_ids)} tokens)"
        )
    return token_ids


@torch.no_grad()
def generate_chart_notes(
    model: AudioChartModel,
    *,
    audio_path: Path,
    timing: CanonicalTiming,
    cond_vec: np.ndarray,
    grid_dir: Path | None = None,
    overlap: OverlapConfig | None = None,
    range_cfg: GenerationRangeConfig | None = None,
    device: torch.device | None = None,
    decode: DecodeConfig | None = None,
    single_window: bool = False,
    single_window_start_bar: int | None = None,
    reference_notes: list[ManiaNote] | None = None,
    build_grid_if_missing: bool = True,
    on_progress: Callable[[int, int], None] | None = None,
) -> tuple[list[ManiaNote], GenerationReport]:
    """Generate notes for a chart; default range is full audio (``audio_full``).

    ``audio_path`` must be an audio file (not a set directory).
    """
    overlap = overlap or OverlapConfig()
    range_cfg = range_cfg or GenerationRangeConfig()
    decode = decode or DecodeConfig()
    device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
    grid_dir = grid_dir or audio_grid_dir()

    audio_file = Path(audio_path)
    stem = resolve_grid_stem(audio_file, timing)
    try:
        grid, meta = load_audio_grid(grid_dir, stem)
    except FileNotFoundError:
        if not build_grid_if_missing:
            raise
        features, meta = compute_audio_grid(audio_file, timing)
        save_audio_grid(features, meta, grid_dir)
        grid = features

    audio_start, audio_end = meta.audio_bar_range
    chart_start: int | None = None
    chart_end: int | None = None
    if reference_notes is not None:
        chart_start, chart_end = chart_event_bar_range(reference_notes, timing)
    gen_start, gen_end = resolve_generation_bar_range(
        audio_start_bar=audio_start,
        audio_end_bar=audio_end,
        chart_start_bar=chart_start,
        chart_end_bar=chart_end,
        cfg=range_cfg,
    )

    if gen_start >= gen_end:
        windows: list[InferenceWindow] = []
        assemble_start, assemble_end = gen_start, gen_end
    elif single_window:
        start = single_window_start_bar if single_window_start_bar is not None else gen_start
        if start < gen_start or start >= gen_end:
            raise ValueError(
                f"single-window start_bar {start} is outside generation range "
                f"[{gen_start}, {gen_end})"
            )
        win_end = start + overlap.window_bars
        keep_end = min(win_end, gen_end)
        windows = [InferenceWindow(start, win_end, start, keep_end, 0)]
        assemble_start, assemble_end = start, keep_end
    else:
        windows = overlap_inference_bar_windows(gen_start, gen_end, overlap)
        assemble_start, assemble_end = gen_start, gen_end

    report = GenerationReport(
        audio_start_bar=audio_start,
        audio_end_bar=audio_end,
        gen_start_bar=gen_start,
        gen_end_bar=gen_end,
        chart_start_bar=chart_start,
        chart_end_bar=chart_end,
        range_mode=range_cfg.mode,
    )

    model = model.to(device)
    cond_t = torch.tensor(cond_vec, dtype=torch.float32, device=device)
    vocab = build_vocab()
    id_to_token = invert_vocab(vocab)
    committed_bars: dict[int, list[int]] = {}
    total_windows = len(windows)
    if on_progress is not None:
        on_progress(0, total_windows)

    for i, win in enumerate(windows):
        window_bars = win.win_end - win.win_start
        audio_slice, _ = slice_audio_window(grid, meta, win.win_start, win.win_end)
        report.audio_ticks_per_window = audio_slice.shape[0]

        initial = initial_row_from_committed_bars(
            committed_bars, gen_start=gen_start, win_start=win.win_start, vocab=vocab
        )
        prompt_ids = (
            build_context_prompt_token_ids(
                committed_bars=committed_bars,
                win_start=win.win_start,
                keep_start=win.keep_start,
                initial_row=initial,
                vocab=vocab,
            )
            if win.context_bars > 0
            else None
        )
        token_ids = generate_window_tokens(
            model,
            audio=torch.tensor(audio_slice, dtype=torch.float32),
            cond_vec=cond_t,
            initial_row=initial,
            window_bars=window_bars,
            device=device,
            decode=decode,
            max_seq_len=overlap.max_seq_len,
            prompt_token_ids=prompt_ids,
            vocab=vocab,
            id_to_token=id_to_token,
        )
        missing = commit_keep_bar_tokens(
            committed_bars,
            token_ids=token_ids,
            id_to_token=id_to_token,
            win_start=win.win_start,
            keep_start=win.keep_start,
            keep_end=win.keep_end,
        )
        if missing:
            raise RuntimeError(
                f"window win=[{win.win_start},{win.win_end}) keep=[{win.keep_start},{win.keep_end}) "
                f"missing keep bars {missing}"
            )
        report.windows.append(
            WindowGenerationLog(
                win_start=win.win_start,
                win_end=win.win_end,
                keep_start=win.keep_start,
                keep_end=win.keep_end,
                context_bars=win.context_bars,
                is_first_window=win.context_bars == 0,
                bars_kept=win.keep_end - win.keep_start,
            )
        )
        logger.info(
            "window win=[%d,%d) keep=[%d,%d) ctx=%d first=%s bars_kept=%d",
            win.win_start,
            win.win_end,
            win.keep_start,
            win.keep_end,
            win.context_bars,
            win.context_bars == 0,
            win.keep_end - win.keep_start,
        )
        if on_progress is not None:
            on_progress(i + 1, total_windows)

    chart_token_ids = assemble_chart_token_ids(
        committed_bars, gen_start=assemble_start, gen_end=assemble_end, vocab=vocab
    )
    chart_tokens = [id_to_token[i] for i in chart_token_ids]
    decoded = tokens_to_notes(chart_tokens, timing, start_bar=assemble_start)
    if decoded.issues:
        report.decode_issues = [
            {"kind": i.kind, "tick": i.tick, "col": i.col} for i in decoded.issues
        ]
        logger.warning(
            "decode issues (%d): %s",
            len(decoded.issues),
            report.decode_issues,
        )
    notes = decoded.notes
    if notes:
        bars = chart_event_bar_range(notes, timing)
        report.first_note_bar = bars[0]
        report.last_note_bar = bars[1] - 1
    if chart_end is not None and report.last_note_bar is not None:
        report.tail_note_count = sum(
            1
            for n in notes
            if split_tick(ms_to_tick(n.time_ms, timing))[0] > chart_end
        )

    if report.windows:
        w0 = report.windows[0]
        logger.info(
            "generation summary: audio=[%d,%d) gen=[%d,%d) first_keep_start=%d first_note_bar=%s "
            "audio_ticks_per_window=%d",
            audio_start,
            audio_end,
            gen_start,
            gen_end,
            w0.keep_start,
            report.first_note_bar,
            report.audio_ticks_per_window,
        )

    return notes, report
