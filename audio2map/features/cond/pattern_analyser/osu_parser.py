...
注意：由于解析逻辑和需要的数据与 ``audio2map.osu.parser`` 相差较大，pattern
分析使用本独立 ``osu_parser.py``（对齐 Converter.fs），**不要**把训练/token/导出
路径接到这里。权威解析见 ``audio2map.osu.parser`` / ``docs/ARCHITECTURE.md``。
"""
