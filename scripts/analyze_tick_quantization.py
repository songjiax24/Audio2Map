...
from audio2map.osu.grid_config import TICKS_PER_BEAT
...
def _fast_parse_endpoints(text: str) -> tuple[float, int, np.ndarray] | None:
    ...
    general = _kv_lines(sections.get("[General]", []))
    diff = _kv_lines(sections.get("[Difficulty]", []))
    try:
        if int(general.get("Mode", -1)) != 3:
            return None
        cs = float(diff["CircleSize"])
        if not (abs(cs - 4.0) < 1e-6 or round(cs) == 4):
            return None
    ...
    if not beat_lengths or offset_ms is None:
        return None
    if len(set(round(b, 6) for b in beat_lengths)) != 1:
        return None
    if len(set(meters)) != 1 or meters[0] != 4:
        return None

    original_bpm = beat_length_to_bpm(beat_lengths[0])
    times: list[int] = []
    for line in sections.get("[HitObjects]", []):
        ...
        try:
            ...
            continue
        times.append(t0)
        if type_bits & 128 and len(parts) > 5:
            head = parts[5].split(":")[0]
            try:
                t1 = int(head)
                if t1 > t0:
                    times.append(t1)
...
def main() -> None:
    ...
    report = {
        ...
        "training_code": (
            "row_tokens.ms_to_tick uses CanonicalTiming.tick_ms (canonical BPM); "
            "quantize_notes snaps notes to that grid before tokenization"
        ),
