...
from audio2map.data.chart_filter import FilterReason, REQUIRED_METER, check_osu_path
...
from audio2map.osu.row_tokens import beatmap_to_row_tokens, build_vocab
...
def main() -> None:
    ...
    for path in tqdm(paths, desc="analyze", unit="file"):
        ...
        if not result.eligible:
            ...
            removed[key] += 1
            if result.reason == FilterReason.METER_NOT_4 and timing.meter_primary is not None:
                meter_not_4_hist[timing.meter_primary] += 1
            continue

        eligible += 1
        try:
            bm = parse_beatmap(path)
            meta = timing_metadata(bm)
            orig_bpm_hist[round(meta["original_bpm"])] += 1
            canon_bpm_hist[round(meta["canonical_bpm"])] += 1
            scale_exp_hist[meta["bpm_scale_exp"]] += 1

            tokens = beatmap_to_row_tokens(bm)
            token_lens.append(len(tokens))
            note_counts.append(bm.note_count)
            hr = hold_ratio(bm)
            hold_ratios.append(hr)
            from audio2map.osu.row_tokens import CanonicalTiming

            ct = CanonicalTiming.from_beatmap(bm)
            hold_counts.append(sum(1 for n in bm.notes if n.note_type.value == "hold"))
            hold_covs.append(hold_coverage(bm, ct))
            if bm.notes:
                from audio2map.osu.row_tokens import all_event_ticks, chart_event_bar_range
    ...
    _print_dist("hold_ratio", hold_ratios)
    _print_dist("hold_coverage", hold_covs)
