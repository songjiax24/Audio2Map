...
from audio2map.model.model import AudioChartModel, build_model, load_checkpoint
...
def test_build_model_defaults() -> None:
    m = build_model(d_model=32, encoder_layers=2, decoder_layers=2, n_heads=4)
    assert isinstance(m, AudioChartModel)
    assert m.architecture == "enc_dec"
    assert len(m.audio_encoder.layers) == 2
    assert len(m.chart_decoder.layers) == 2
