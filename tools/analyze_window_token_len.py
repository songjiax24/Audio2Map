...
from audio2map.dataset.windows import frame_window_tokens
from audio2map.grid import CanonicalTiming, chart_event_bar_range
...
# Candidate MAX_DECODER_LEN values for overflow-coverage reporting.
_DECODER_LEN_LIMITS: tuple[int, ...] = (1024, 2048, 4096)


def _audio_bar_range(path: Path, timing: CanonicalTiming, *, grid_dir: Path) -> tuple[int, int] | None:
    try:
        audio_path = chart_audio_path(path)
...
def _window_token_len(beatmap, timing: CanonicalTiming, start_bar: int) -> int:
    ...
    return len(
        frame_window_tokens(
            chart,
            initial_row_at_bar(beatmap.notes, timing, start_bar=start_bar),
        )
    )
...
def main() -> None:
    ...
    report = {
        ...
        "overflow_coverage_sampled": {
            str(limit): _overflow_ratio(sampled_lens, limit) for limit in _DECODER_LEN_LIMITS
        },
        "overflow_coverage_per_chart_max": {
            str(limit): _overflow_ratio(max_per_chart, limit) for limit in _DECODER_LEN_LIMITS
        },
    }
    ...
    recommended = max(_DECODER_LEN_LIMITS)
    for limit in _DECODER_LEN_LIMITS:
